...
rke安装在172.18.44.151上  /home/rke   用rke用户执行命令
centos7.5
docker 18.06
kubernetes 1.11.1
rancher 2.0.8
...
rke下载地址`https://github.com/rancher/rke/releases/`

经实验，v0.1.09版本执行无bug，v0.1.10版本执行有问题

```shell
chmod +x rke_linux-amd64		# 添加可执行权限
./rke_linux-amd64 --version		# 查看版本号
```
...
centos系统不能用root运行rke执行文件，需要新建用户并建立免密码连接

```shell
useradd rke
usermod -aG docker rke		# rke用户需要docker权限，添加到docker组
su rke
cd ~
ssh-keygen -t rsa		# 生成密钥，直接按回车，不需要输入任何文字
cat ~/.ssh/id_rsa.pub > ~/.ssh/authorized_keys		# 改文件所属为rke，权限为600
chmod 600 ~/.ssh/authorized_keys	
scp ~/.ssh/authorized_keys 172.18.44.151:~/.ssh/authorized_keys
ssh 172.18.44.151			# 测试免密码连接
```
...
openssl req -newkey rsa:4096 -nodes -sha256 -keyout ca.key -x509 -days 365 -out ca.crt
    ...
    Common Name(e.g. server FQDN or YOUR name)[]: t2.rke.com
...
```
openssl req -newkey rsa:4096 -nodes -sha256 -keyout t2.rke.com.key -out t2.rke.com.csr
...
```
openssl x509 -req -days 365 -in t2.rke.com.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out t2.rke.com.crt
```
...
```
cat t2.rke.com.crt | base64 -w0
```

###### 2.cluster.yml文件

之前在配置rancher时将rancher镜像写成rancher/rancher:latest，k8s集群起不来，后改成rancher/rancher:v2.0.8，可正常运行。

```yaml
nodes:
  - address: 172.18.44.150
    user: rke
    role: [controlplane,etcd,worker]
    ssh_key_path: ~/.ssh/id_rsa
  - address: 172.18.44.151
    user: rke
    role: [controlplane,etcd,worker]
    ssh_key_path: ~/.ssh/id_rsa
  - address: 172.18.44.152
    user: rke
    role: [controlplane,etcd,worker]
    ssh_key_path: ~/.ssh/id_rsa
...
```shell
./rke_linux-amd64 up --config cluster.yml			# 创建集群
./rke_linux-amd64 remove --config cluster.yml		# 删除集群
```
